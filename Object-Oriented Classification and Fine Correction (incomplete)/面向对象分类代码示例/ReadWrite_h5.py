...
if __name__ == '__main__':
    ...
    """h5_path = r"L:\20200310\GF5_DPC_20200310_009779_L10000031114_B865.h5"
    ...
    # get sinusoidal tif
    writ_tif2(dataset_arr, sinpro, transform, r'E:\按键精灵\result.tif')"""

    # prosrs, geosrs = defineSRS(4326)
    # paths = glob.glob(r'E:\按键精灵\*.tif')
    # for path in paths:
    #     Idataset = get_tifDataset(path)
    #     resampling(Idataset, os.path.join(r'E:\SteamLibrary', os.path.basename(path)), prosrs, 1, nodata=-32767)

    pro, geo = defineSin()
    print(pro)

    tifPath = r"J:\发送文件\Atmosphere\SLIC\ENVIresult\231031-230128_erfenlei.tif"
    tifDataset = get_tifDataset(tifPath)
    projection, transform = get_GeoInformation(tifDataset)
    imgx, imgy = get_RasterXY(tifDataset)
    tifArr = get_RasterArr(tifDataset, imgx, imgy)
    tifArr[tifArr != -1] = 0
    write_tif(tifArr, projection, transform, r"J:\发送文件\Atmosphere\SLIC\ENVIresult\231031-230128_erfenlei_class.tif")

    exit()
    for tifPath in glob.glob(r'J:\发送文件\jjz\result\*.tif'):
        dataset = get_tifDataset(tifPath)
        print(dataset)
        pro, trans = get_GeoInformation(dataset)
        resampling(dataset,
                   os.path.join(r'J:\发送文件\jjz\result\resample',
