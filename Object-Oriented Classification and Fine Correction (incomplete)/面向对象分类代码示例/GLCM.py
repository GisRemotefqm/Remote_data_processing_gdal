...
if __name__ == '__main__':
    ...
    path = r".\data\Resample\231031_resample_clip.tif"
    ...
    rb_glcm = fast_glcm(band3, vmin=int(np.min(band3)), vmax=int(np.max(band3)), levels=10, kernel_size=7, angle=0)

    rb_mean = fast_textural_features(rb_glcm, levels=7, kernel_size=7)[2]

    write_tif(rb_mean, pro, trans, os.path.join(r'.\data\GLCM', os.path.basename(path)[:-4] + '_glcmMEANBand4.tif'))
