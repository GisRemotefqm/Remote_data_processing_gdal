...
class RBM:
	...
	def sample_h(self, x):
		...
		if self.mode == 'bernoulli':
			...
			return p_h_given_v, torch.add(p_h_given_v, torch.normal(mean=0, std=1, size=p_h_given_v.shape).to(self.device))
	...
	def sample_v(self, y):
		...
		if self.mode == 'bernoulli':
			...
			return p_v_given_h, torch.add(p_v_given_h, torch.normal(mean=0, std=1, size=p_v_given_h.shape).to(self.device))
