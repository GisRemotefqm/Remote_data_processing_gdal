...
if __name__ == '__main__':
    ...
    path = r'J:\毕业论文\TOA数据\230918\自己做的\定标_step2'
    tif_dirs = os.listdir(path)
    for tif_dir in tif_dirs:
        if os.path.splitext(tif_dir)[1] == '.tif':
            tif_path = os.path.join(path, tif_dir)
            ...
            rb, grb = math_rb_grb(tif_arr)
            # write_tif(rb, pro, trans, os.path.join(r'J:\毕业论文\TOA数据\230918\数据推送CZ\去云_step3\GF1\2020', tif_dir[:-4] + '_rb.tif'))
            # write_tif(grb, pro, trans, os.path.join(r'J:\毕业论文\TOA数据\230918\数据推送CZ\去云_step3\GF1\2020', tif_dir[:-4] + '_grb.tif'))

            rb_glcm = fast_glcm(rb, vmin=int(np.min(rb)), vmax=int(np.max(rb)), levels=10, kernel_size=7, angle=0)
            grb_glcm = fast_glcm(grb, vmin=int(np.min(grb)), vmax=int(np.max(grb)), levels=10, kernel_size=7, angle=0)

            rb_mean = fast_textural_features(rb_glcm, levels=8, kernel_size=2)[2]
            grb_mean = fast_textural_features(grb_glcm, levels=8, kernel_size=2)[2]
            ...
            query_result = np.argwhere(((tif_arr[0, :, :] > 68) & ((rb <= -37) & (grb <= -10))) |
                                       ((tif_arr[0, :, :] > 68) & ((((rb_mean >= 2) & (rb_mean <= 3)) & ((rb_mean >= 1.5) & (rb_mean <= 1.8)) & ((rb_mean >= 0.5) & (rb_mean <= 0.7))) & ((((grb_mean >= 0) & (grb_mean <= 0.77)))))))

            for i in range(query_result.shape[0]):
                tif_arr[:, query_result[i, 0], query_result[i, 1]] = 0
            # write_tif(rb_mean, pro, trans, os.path.join(r'J:\毕业论文\TOA数据\230918\数据推送CZ\去云_step3\GF1\2020', tif_dir[:-4] + '_rb_mean.tif'))
            # write_tif(grb_mean, pro, trans, os.path.join(r'J:\毕业论文\TOA数据\230918\数据推送CZ\去云_step3\GF1\2020', tif_dir[:-4] + '_grb_mean.tif'))
            write_tif(tif_arr, pro, trans, os.path.join(r'J:\毕业论文\TOA数据\230918\自己做的\去云', tif_dir[:-4] + '_mask_cloud.tif'))
